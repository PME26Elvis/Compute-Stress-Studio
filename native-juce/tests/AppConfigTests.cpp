...
GPU_TEST_CASE("explicit command line values override defaults") {
    const auto parsed = parseArguments({"--duration", "7200", "--load=75", "--memory-mib",
                                        "256", "--device", "1", "--window-ms", "250",
                                        "--kernel-ms", "12", "--background"});
    GPU_REQUIRE(parsed.ok);
...
GPU_TEST_CASE("removed monitoring and output options are rejected") {
    for (const auto* option : {"--temp-limit", "--output-dir", "--no-telemetry"}) {
        const auto parsed = parseArguments({option, "80"});
        GPU_REQUIRE(!parsed.ok);
    }
}
...
GPU_TEST_CASE("quoted command line values remain one token") {
    const auto tokens = splitCommandLine("--device \"1\" --dry-run");
    GPU_REQUIRE(tokens.size() == 3);
    GPU_REQUIRE(tokens[1] == "1");
